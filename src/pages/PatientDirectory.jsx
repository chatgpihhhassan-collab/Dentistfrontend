    ...
    const handleExportLogsPDF = async () => {
        ...
        try {
            ...
            const drawFooter = () => {
                ...
                pdf.roundedRect(margin, pageHeight - 21, contentWidth, 13, 1, 1, 'FD');

                const pRefNo = selectedPatient?.referenceNumber || ('DEN-2026-' + (selectedPatient?.patientID || '00000'));
                const pDob = selectedPatient?.dob ? new Date(selectedPatient.dob).toLocaleDateString('en-GB') : 'Verified DOB on file';

                pdf.setFont('helvetica', 'bold');
                pdf.setFontSize(6.8);
                pdf.setTextColor(11, 79, 74);
                pdf.text(`PATIENT HEALTH PORTAL: https://dentistfrontend.vercel.app/portal/login  |  USER / REF #: ${pRefNo}`, margin + 3, pageHeight - 15);

                pdf.setFont('helvetica', 'normal');
                pdf.setFontSize(6.2);
                pdf.setTextColor(100, 116, 139);
                pdf.text(`Password: Log in using Reference # or activate with verified Date of Birth (${pDob}) at portal/activate.`, margin + 3, pageHeight - 10.5);

                pdf.setFontSize(6);
                pdf.setTextColor(148, 163, 184);
                pdf.text('Confidential Clinical Audit Record · Generated by Dentia Dental Practice Management', margin, pageHeight - 4);
            };
